...
class Consumable(BaseComponent):
    ...
    def activate(self, action:actions.ItemAction) -> None:
        # Invoke item ability. 'action' is context
...
class ConfusionConsumable(Consumable):
    ...
    def activate(self, action:actions.ItemAction) -> None:
        ...
        self.engine.message_log.add_message(
            f"The eyes of the {target.name} look vacant and it begins to stumble.",
...
class HealingConsumable(Consumable):
    ...
    def activate(self, action:actions.ItemAction) -> None:
        ...
        if amount_recovered > 0:
            self.engine.message_log.add_message(
                f"You use the {self.parent.name}, and recover {amount_recovered} HP.", color.health_recovered,
            )
            self.consume()
        else: raise Impossible(f"You are not injured.")
...
class LightningDamageConsumable(Consumable):
    ...
    def activate(self, action:actions.ItemAction) -> None:
        ...
        if target:
            ...
            raise Impossible("No visible enemy in range.")
